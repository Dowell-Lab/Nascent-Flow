...
GC=$(unzip -c "$(find . -maxdepth 1 -name "$SRR""_fastqc.zip")" "$SRR"_fastqc/fastqc_data.txt \
         | grep "%GC" | grep -o "[0-9]*")
SEQ=$(unzip -c "$(find . -maxdepth 1 -name "$SRR""_fastqc.zip")" "$SRR"_fastqc/fastqc_data.txt | \
          grep "Total Sequences" | \
          grep -o "[0-9]*")
DEDUP=$(unzip -c "$(find . -maxdepth 1 -name "$SRR""_fastqc.zip")" "$SRR"_fastqc/fastqc_data.txt | \
